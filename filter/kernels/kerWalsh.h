...
SYNC1:
		...
		for(arrIdx = 0; HODGES_SORT_ARR_SIZE > arrIdx; arrIdx+=THREADS_PER_PIXEL){	//if array is longer than blockDim
			if(thread < gpuImageSize){		//if 
				...
				if(sortArr[HODGES_SORT_ARR_SIZE+2]&&sortArr[HODGES_SORT_ARR_SIZE+3]) goto SYNC2; //both medians found, end
...
#define WBES_THREADS_PER_PIXEL (32)
	// +4 for control variables
#define INT32_ALIGNED_WBES_SORT_ARR_SIZE \
	(((HODGES_SORT_ARR_SIZE+8)*sizeof(imDataType))/4+1)

/*
	-- HODGES_SORT_ARR_SIZE to HODGES_SORT_ARR_SIZE+3 contain medians and quartils
	-- HODGES_SORT_ARR_SIZE+4 to HODGES_SORT_ARR_SIZE+7 contain control variables  

*/

template<typename imDataType>
__global__ void GPUwbesOpt(imDataType* dst, int seIndex, imDataType* srcA){
	...
	unsigned char *walshIdxArr = (unsigned char*)&wList[gpuCap[seIndex]];
		//each warp (generally group of threads) has its own sortArr
	imDataType *sortArr = (imDataType*)&wList[gpuCap[seIndex] + INT32_ALIGNED_WALSH_INDEXES_SIZE 
			+ INT32_ALIGNED_WBES_SORT_ARR_SIZE*(threadIdx.x/WBES_THREADS_PER_PIXEL)];
	unsigned arrIdx, lesser, bigger;
	unsigned q = ceil((float)HODGES_SORT_ARR_SIZE/4)-1;	//index of 1st quartil
	...
	__syncthreads();
		...
		//process sequentially as much pixels as is block size
		//more than one pixel is processed at a time
	for(unsigned i=0;i<blockDim.x; i += blockDim.x/WBES_THREADS_PER_PIXEL){	//jump over few pixels
		thread = i + blockIdx.x*blockDim.x + threadIdx.x/WBES_THREADS_PER_PIXEL;
		if(thread < gpuImageSize){
			arrIdx = MAP_THREADS_ONTO_IMAGE(thread);
			WALSH_NB_TO_SHARED_PARALELL(thread, bigger);
				//arrIdx not used until storage of the result -> use it as dummy variable
		...
		//}
		if(thread < gpuImageSize){
			for(thread = 0, arrIdx = threadIdx.x%WBES_THREADS_PER_PIXEL; 
			 HODGES_SORT_ARR_SIZE-gpuCap[seIndex] > thread; 
			 thread += WBES_THREADS_PER_PIXEL, arrIdx += WBES_THREADS_PER_PIXEL){
					//store averages behind nbpixel values
				if(arrIdx >= HODGES_SORT_ARR_SIZE-gpuCap[seIndex]) goto SYNC1;
				sortArr[arrIdx + gpuCap[seIndex]] =
					((unsigned)sortArr[walshIdxArr[2*(arrIdx)+1]]
...
SYNC1:
		__syncthreads();
			//find median
			//use arridx as dummy instead, thread is reserved for overflow handling
		sortArr[HODGES_SORT_ARR_SIZE+4] = sortArr[HODGES_SORT_ARR_SIZE+5] = 
			sortArr[HODGES_SORT_ARR_SIZE+6] = sortArr[HODGES_SORT_ARR_SIZE+7] = 0;

		for(arrIdx = 0; HODGES_SORT_ARR_SIZE > arrIdx; arrIdx+=WBES_THREADS_PER_PIXEL){	//if array is longer than blockDim
			if(thread < gpuImageSize){		//if 
					//terminate excessive threads
				thread = (threadIdx.x%WBES_THREADS_PER_PIXEL) + arrIdx;
				if(thread >= HODGES_SORT_ARR_SIZE) goto SYNC2; 
				if(sortArr[HODGES_SORT_ARR_SIZE+4]&&sortArr[HODGES_SORT_ARR_SIZE+5]&&
					sortArr[HODGES_SORT_ARR_SIZE+6]&&sortArr[HODGES_SORT_ARR_SIZE+7]) goto SYNC2; //all values found, end
				imDataType curElement = sortArr[thread];
				...
				if(!((lesser>HODGES_SORT_ARR_SIZE/2)||(bigger>HODGES_SORT_ARR_SIZE/2))){
					if((lesser<=HODGES_SORT_ARR_SIZE/2-1+ODD)&&(bigger<=HODGES_SORT_ARR_SIZE/2)){
						sortArr[HODGES_SORT_ARR_SIZE  ] = curElement;
						sortArr[HODGES_SORT_ARR_SIZE+4] = 1;
					}
					if((lesser<=HODGES_SORT_ARR_SIZE/2)&&(bigger<=HODGES_SORT_ARR_SIZE/2-1+ODD)){ 
						sortArr[HODGES_SORT_ARR_SIZE+1] = curElement;
						sortArr[HODGES_SORT_ARR_SIZE+5] = 1;
					}
				}
					//quartils
				if(!(bigger>q && lesser>q)){
					if((lesser<=q)&&(bigger<HODGES_SORT_ARR_SIZE-q)){ 
						sortArr[HODGES_SORT_ARR_SIZE+2] = curElement;
						sortArr[HODGES_SORT_ARR_SIZE+6] = 1;
					}
					if((bigger<=q)&&(lesser<HODGES_SORT_ARR_SIZE-q)){ 
						sortArr[HODGES_SORT_ARR_SIZE+3] = curElement;
						sortArr[HODGES_SORT_ARR_SIZE+7] = 1;
					}
				}
				/*if(lesser<HODGES_SORT_ARR_SIZE-q){
					if(lesser<=HODGES_SORT_ARR_SIZE/2){
						if(lesser<=HODGES_SORT_ARR_SIZE/2-1+ODD){
							if((lesser<=q)&&(bigger<HODGES_SORT_ARR_SIZE-q)){ 
								sortArr[HODGES_SORT_ARR_SIZE+2] = curElement;
								sortArr[HODGES_SORT_ARR_SIZE+6] = 1;
							}else if(bigger<=HODGES_SORT_ARR_SIZE/2){
								sortArr[HODGES_SORT_ARR_SIZE  ] = curElement;
								sortArr[HODGES_SORT_ARR_SIZE+4] = 1;
							}
						}else if(bigger<=HODGES_SORT_ARR_SIZE/2-1+ODD){ 
							sortArr[HODGES_SORT_ARR_SIZE+1] = curElement;
							sortArr[HODGES_SORT_ARR_SIZE+5] = 1;
						}
					}else if(bigger<=q){ 
...
SYNC2:
		...
		if(thread < gpuImageSize){
			if((threadIdx.x%WBES_THREADS_PER_PIXEL) == 0){
				thread = i + blockIdx.x*blockDim.x + threadIdx.x/WBES_THREADS_PER_PIXEL;
				arrIdx = MAP_THREADS_ONTO_IMAGE(thread);
				dst[arrIdx] = ((unsigned)sortArr[HODGES_SORT_ARR_SIZE]+sortArr[HODGES_SORT_ARR_SIZE+1]+
					sortArr[HODGES_SORT_ARR_SIZE+2]+sortArr[HODGES_SORT_ARR_SIZE+3])/4;
			}
			thread = 0; //to ensure that condition works
		}
		__syncthreads();
